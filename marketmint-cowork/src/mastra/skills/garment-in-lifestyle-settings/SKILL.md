---
name: garment-in-lifestyle-settings
description: Use only when the user explicitly asks for garment-on-model in a lifestyle or outdoor setting (e.g. "lifestyle images for this garment", "outdoor setting", "beach", "street", "café"). Do not use this skill for generic image generation requests involving garments. If the user has mentioned that they want to put a garment on a model without specifying the type of background they want to use (lifestyle/outdoor or indoor/studio), you should first clarify with the user about their background preference. Once the user clarifies their background preference and prefers a lifestyle background then you can select this skill. Do not use for generic image generation requests involving garments like  "marketing images for this garment".
workflow_id: garments_v2
use_case_id: garments_v2_lifestyle
---

# Garment in LifeStyle Scene — Guidelines

This skill covers user requests for putting garments on models in lifestyle (outdoor) settings.

Example requests:
- "Put this dress on a model standing in front of a beach."
- "Try this jacket on a male model in a street-style scene."
- "Place my product on a model in a lifestyle background."
...
```jsonc
{
  ...
  "arguments": {
    ...
    "workflow_inputs": {
      ...
      "user_query": "Put this shirt on a male model in a lifestyle setting.",
...
- If the request is not about a model wearing a garment in a lifestyle setting, do **not** use this skill.
