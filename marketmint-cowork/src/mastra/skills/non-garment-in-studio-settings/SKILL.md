---
name: non-garment-in-studio-settings
description: 'Use ONLY when the user explicitly asks for a non-garment PRODUCT (object: bag, helmet, shoe, bottle, etc.) in a studio or indoor setting (e.g. "studio background", "clean backdrop", "product on white"). Do NOT use for animals (horse, dog, etc.) or for "studio images of X" when X is not a product—use creative-generation for those. If the user wants a model with a product but has not said lifestyle vs studio, clarify first; once they prefer studio, select this skill. Do not use for generic requests like "marketing images for this shoe" without studio context.'
workflow_id: non_garment_shoot
use_case_id: non_garment_in_studio_background
---

# Non-Garment in Studio — Guidelines

This skill covers user requests for generating images of non-garment products in studio/indoor settings.

Example requests:
- "Generate studio images for this helmet"
- "Make a model hold this bag in a studio setting"
...
```jsonc
{
  ...
  "arguments": {
    ...
    "workflow_inputs": {
      ...
      "custom_description": "User wants to use a non-garment studio theme."
    }
  }
}
```
...
- If the request is not about studio/indoor images for non-garment products, do **not** use this skill.
